...
var api = {};
if(fs.existsSync(path.join(BASE, 'build', 'api.json'))) {
    api = JSON.parse(fs.readFileSync(path.join(BASE, 'build', 'api.json')));
}
...
router.all('/:group/:action', function(req, res) {
    ...
    var actionPath = path.join(__root, 'service', req.params.group);
    if(config.devmode) {
        try {
            var key = require.resolve(actionPath);
            var mtime = fs.statSync(actionPath + '.js').mtime.getTime();
            if (require.cache[key].__DEV_REQ_TIME__ != mtime) {
                delete require.cache[key];
            }
            if(!require.cache[key] || !require.cache[key].__DEV_REQ_TIME__) {
                setTimeout(function() {
                    try {
                        require.cache[key].__DEV_REQ_TIME__ = mtime;
                    } catch(e) {}
                }, 10);
            }
        } catch(e) {
    ...
    action.apply(TMP = {
        ...
        result : function(data) {
            ...
            if (req.query.jsoncallback && req.query.jsoncallback.length > 0) {
                ...
                var theDMB = dataMatchBean.dataMatchBean(theText, actionInfo.result);

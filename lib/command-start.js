...
module.exports = function(args) {
    fs.mkdir(path.join(BASE, 'log'), function() {
        if(args.daemon) {
            ...
            // rotate log stream 
            // keep log of one day (or 1G) and a backup of previous day (or 1G)
            var log = {
            ...
            (function rotate() {
                ...
                function rotateStream(streamName, fname) {
                    ...
                    fs.stat(stream.fpath, function(err, stat) {
                        if (err || stat.size > 1073741824 || stat.ctime.getTime() < expire) {
            ...
            if (config.autoUpdateInterval) {
                function update() {
                    require('./app/update-site')(function(updated) {
                        if (updated) {
                            log.out.write('Server updated!');
                            log.err.write('Server updated!');
                            try {
                                if (child) child.kill()
                            } catch(e) {}
                        }
                ...
                var interval = config.autoUpdateInterval - 0 || 0;
                if (interval < 1000)
                    interval = 1000;
                update();
